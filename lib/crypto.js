...
// Cryptographically random, URL-safe, zero collision probability
...
/**
 * VTPass Specific Request ID format:
 * YYYYMMDDHHII (12 chars, Africa/Lagos) + alphanumeric string
 */
const generateVTPassRef = (prefix = "") => {
    const d = new Date();
    ...
    const m = {};
    for (let p of parts) m[p.type] = p.value;

    // Ensure hour is '24' instead of '00' edge cases if they happen, though en-GB usually does 00-23
    let hour = m.hour === '24' ? '00' : m.hour;

    const timePrefix = `${m.year}${m.month}${m.day}${hour}${m.minute}`;
    const random = crypto.randomBytes(6).toString('hex');

    return `${timePrefix}${prefix}${random}`;
};

module.exports = { encrypt, decrypt, generateRef, generateVTPassRef };